...
| WebApp | Servicio de internacionalización propio (`I18nService`) y pipe `t` con diccionarios de traducción EN/ES, persistencia del idioma en localStorage y sincronización del atributo `lang`. | Etiquetas ARIA en búsquedas y acciones, estados `aria-expanded`/`aria-pressed`, avisos `role="alert"` y `aria-live`, diálogos con `aria-modal`, textos alternativos de productos y títulos para mapas embebidos. |
| Platform API | Códigos de error estructurados y mensajes de excepción en español (e.g. en excepciones de reglas de negocio), delegando la localización visual de los textos al frontend (`I18nService`). | No aplica una interfaz visual. La API aporta contratos HTTP y errores estructurados; la accesibilidad se evalúa en Website/WebApp y en la documentación Swagger consumida por personas desarrolladoras. |
