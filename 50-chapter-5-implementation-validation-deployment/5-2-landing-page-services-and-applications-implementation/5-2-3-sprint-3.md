...
La versión de referencia del backend para AV2 es `v1.0.0`, construida con Java Spring Boot REST API, Domain-Driven Design, modular monolith, bounded contexts, Shared Kernel, JPA, PostgreSQL, controladores REST, commands, queries, infrastructure y ajustes de autenticación IAM alineados con la Web Application. Este incremento prioriza los flujos core de Catalog Management, Warehouse y Sales como base para revisión académica y validación controlada, sin declarar operación productiva, integración total con toda la Web Application, reemplazo completo de todos los mocks ni cobertura total del roadmap backend.
...
El Sprint Backlog 3 concentra el trabajo asociado al primer corte AV2 frontend/backend y a la consolidación de la base backend de Nexa Platform. Su objetivo principal fue establecer una base técnica coherente para la primera Web Services API, organizada por bounded contexts, con controladores iniciales, Shared Kernel, patrones de repositorio, Unit of Work, persistencia mediante JPA/PostgreSQL para el despliegue controlado y trazabilidad de release para revisar la transición desde mocks hacia backend real.
...
| Sprint 3 | Task | Foundation backend con DDD | NX-116 | TS-NX-003-002 - Scaffold Spring Boot y DDD | Preparar la estructura inicial Spring Boot y la organización Domain-Driven Design del backend. | 8.0 | Gerard Gianpier Rojas Mancilla | Finalizado |
| Sprint 3 | Task | Persistencia y dominio backend | NX-122 | TS-NX-003-003 - Implementar JPA y entidades de dominio | Incorporar JPA y las entidades de dominio necesarias para la persistencia de la Platform API. | 13.0 | César Marín | Finalizado |
...
| Sprint 3 | Task | Pruebas unitarias de catálogo | NX-262 | TS-NX-003-008 - Configurar JUnit y Mockito para pruebas del módulo de catálogo | Configurar pruebas unitarias Java para cubrir búsqueda, filtros, paginación y acceso por cuenta del módulo de catálogo. | 8.0 | Diego Yucra Sandoval | Finalizado |
...
| Sprint 3 | Task | Backend foundation de Nexa Platform | NX-856 | TS-NX-003-001 - Configurar Java Spring Boot REST API foundation | Preparar Java, Spring Boot, el entrypoint de aplicacion, controladores REST, Swagger/OpenAPI y configuración base para ejecución local. | 8.0 | Diego Yucra Sandoval | Finalizado |
...
| Sprint 3 | Task | Persistencia relacional AV2 | NX-862 | TS-NX-003-007 - Preparar foundation de persistencia relacional | Configurar JPA y la base de persistencia relacional requerida por el despliegue controlado de AV2. | 5.0 | Joaquín Francisco Verde Bueno | Finalizado |
...
| Sprint 3 | Task NX-116 | NX-116 | Scaffold Spring Boot y DDD | NX-120 | Crear modulos Java y referencias del proyecto | Crear los proyectos de la solución y configurar sus referencias. | — | Gerard Gianpier Rojas Mancilla | Finalizado |
...
| Sprint 3 | Task NX-122 | NX-122 | Implementar JPA y entidades de dominio | NX-123 | Codificar entidades Company, User y Order | Implementar las entidades de dominio principales para la persistencia. | — | César Marín | Finalizado |
| Sprint 3 | Task NX-122 | NX-122 | Implementar JPA y entidades de dominio | NX-124 | Configurar entidades JPA y mapeo relacional | Configurar las entidades, repositorios y anotaciones de persistencia requeridas por el modelo relacional. | — | César Marín | Finalizado |
| Sprint 3 | Task NX-122 | NX-122 | Implementar JPA y entidades de dominio | NX-125 | Ejecutar migración InitialCreate | Crear y ejecutar la migración inicial de la base de datos. | — | César Marín | Finalizado |
| Sprint 3 | Task NX-128 | NX-128 | Implementar JWT y autenticación backend | NX-129 | Configurar seguridad JWT en Spring Security | Configurar la validación y autenticación mediante JWT Bearer. | — | Diego | Finalizado |
...
| Sprint 3 | Task NX-856 | NX-856 | Configurar Java Spring Boot REST API foundation | NX-1060 | Inicialización de la Solución | Crear la jerarquía de directorios con DDD y configurar la solución para separar responsabilidades. | 6.0 | Diego | Por hacer |
| Sprint 3 | Task NX-856 | NX-856 | Configurar Java Spring Boot REST API foundation | NX-1061 | Implementación de Swagger | Habilitar Swagger para visualizar y probar endpoints, manteniendo actualizado el contrato de API. | 6.0 | Diego | Por hacer |
...
| Sprint 3 | Task NX-866 | NX-866 | Prepare frontend/backend release version for AV2 | NX-1080 | Verificación de consistencia de contratos | Validar que los DTOs del frontend coincidan con las definiciones del backend. | 6.0 | Diego | Finalizado |
| Sprint 3 | Task NX-866 | NX-866 | Prepare frontend/backend release version for AV2 | NX-1081 | Preparación de perfiles de configuración | Crear perfiles de configuración para frontend y Spring Boot sin valores hardcoded. | 6.0 | César Marín | Finalizado |
...
Release de cierre AV2 disponible para revisión. Este repositorio concentra la base backend con Java Spring Boot REST API, bounded contexts, Shared Kernel, JPA/PostgreSQL, IAM, commands, queries, infrastructure, documentación técnica y release `v1.0.0` con título visible `Nexa Platform - Production Release v1.0.0`. Historial de commits: [https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform/commits/main/](https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform/commits/main/).
...
| `upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform` | `develop` | `d6b123f` | `feat(persistence): generate InitialCreate JPA migrations for PostgreSQL` | Initial persistence migration for PostgreSQL. | 2026-06-15 |
| `upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform` | `develop` | `fb3eeb7` | `fix(persistence): make SeedDataService support idempotent PostgreSQL executions` | Idempotent seed execution support for PostgreSQL review data. | 2026-06-15 |
...
| `upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform` | `develop` | `5cb5962` | `feat(config): add local application properties template` | Local Spring Boot configuration template. | 2026-06-15 |
...
| `upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform` | `develop` | `461ccba` | `refactor(persistence): align JPA configuration with PostgreSQL provider` | Persistence provider alignment for PostgreSQL. | 2026-06-15 |
| `upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform` | `develop` | `3216150` | `feat(persistence): add PostgreSQL connector dependency` | PostgreSQL connector dependency for JPA/Spring Data. | 2026-06-15 |
...
Release de cierre AV2 disponible para revisión. `nexa-website v3.0.0` se registra con el título visible `v3.0.0 - Nexa Landing Website Production Release` e incorpora el cierre de Landing Page, navegación, páginas de detalle, correcciones visuales y documentación de repositorio. Historial de commits: [https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-website/commits/main/](https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-website/commits/main/).
...
El despliegue controlado incluye la Web Application publicada en Render, la Platform API publicada en Render y la base PostgreSQL preparada para el backend. Como referencias de revisión se mantienen los entornos `https://nexa-webapp-fv2v.onrender.com` para la WebApp y `https://nexa-platform.onrender.com` para la Platform API. La evidencia de Swagger/OpenAPI se considera parte del despliegue porque permite validar documentación de endpoints desde la API preparada para revisión.
...
La foundation compartida integró Shared Kernel, repositories, Unit of Work, JPA/PostgreSQL y Swagger/OpenAPI como base técnica común para la evolución backend. La higiene de GitFlow y release permitió separar ramas, consolidar `nexa-platform v1.0.0`, `nexa-website v3.0.0` y `nexa-webapp v2.0.0`, y mantener trazabilidad del incremento sin declarar operación productiva.
