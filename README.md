<div align="center">

<br/>

<img src="./assets/images/chapter-2/competitors/logo-nexa.svg" alt="Nexa" width="220"/>

<br/><br/>

# nexa-report

**Academic report, product research, backlog, architecture documentation, and project evidence for Nexa**

<br/>

![Markdown](https://img.shields.io/badge/Markdown-docs--as--code-0a2540?style=for-the-badge&logo=markdown&logoColor=white)
![Open Source](https://img.shields.io/badge/Open%20Source-audited%20evidence-0EA5E9?style=for-the-badge)
![Architecture](https://img.shields.io/badge/Architecture-DDD%20%7C%20bounded%20contexts-2a67d9?style=for-the-badge)
![Evidence](https://img.shields.io/badge/Evidence-AV2%20release-22c55e?style=for-the-badge)

<br/>

![Course](https://img.shields.io/badge/Course-1ASI0729%20Desarrollo%20de%20Aplicaciones%20Open%20Source-0a2540?style=flat-square)
![Cycle](https://img.shields.io/badge/Cycle-2026--10-0a2540?style=flat-square)
![University](https://img.shields.io/badge/University-UPC-0a2540?style=flat-square)
![Team](https://img.shields.io/badge/Team-Nexa-2a67d9?style=flat-square)
![Status](https://img.shields.io/badge/Status-Release%203.0.0-22c55e?style=flat-square)

<br/>

📘 **[Open Report Repository →](https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-report)**

<br/>

</div>

---

## Overview

`nexa-report` contains the academic evidence for Nexa in the course Desarrollo de Aplicaciones Open Source. It documents the product problem, target segments, requirements, backlog, architecture, database design, implementation evidence, validation, deployment evidence, and annexes.

The report is maintained as Docs-as-Code in Markdown and aligns with the technical repositories `nexa-website`, `nexa-webapp`, and `nexa-platform`.

---

## Repository Map

<table>
  <tr>
    <td width="50%">
      <p><a href="https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-website">upc-pre-202610-1asi0729-12010-VulturesD/nexa-website</a></p>
      <p>Public landing website and central product entry point.</p>
      <p><a href="https://upc-pre-202610-1asi0729-12010-vulturesd.github.io/nexa-website/">Open Live Website</a></p>
      <p>
        <img alt="HTML5" src="https://img.shields.io/badge/HTML5-static-E34F26?style=flat-square&logo=html5&logoColor=white" />
        <img alt="CSS3" src="https://img.shields.io/badge/CSS3-responsive-1572B6?style=flat-square&logo=css3&logoColor=white" />
        <img alt="JavaScript" src="https://img.shields.io/badge/JavaScript-vanilla-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
      </p>
    </td>
    <td width="50%">
      <p><a href="https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-webapp">upc-pre-202610-1asi0729-12010-VulturesD/nexa-webapp</a></p>
      <p>Angular Web Application for operational workflows and buyer-facing coordination.</p>
      <p><a href="https://nexa-webapp-fv2v.onrender.com/login">Open Live WebApp Login</a></p>
      <p>
        <img alt="Angular" src="https://img.shields.io/badge/Angular-21-DD0031?style=flat-square&logo=angular&logoColor=white" />
        <img alt="TypeScript" src="https://img.shields.io/badge/TypeScript-5.9-3178C6?style=flat-square&logo=typescript&logoColor=white" />
        <img alt="Angular Material" src="https://img.shields.io/badge/Angular%20Material-21-0a2540?style=flat-square" />
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <p><a href="https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform">upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform</a></p>
      <p>Spring Boot backend platform and REST API service layer.</p>
      <p><a href="https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-platform/wiki">Open Engineering Wiki</a></p>
      <p>
        <img alt="Java" src="https://img.shields.io/badge/Java-21-0a2540?style=flat-square" />
        <img alt="Spring Boot" src="https://img.shields.io/badge/Spring%20Boot-3.3-6DB33F?style=flat-square&logo=springboot&logoColor=white" />
        <img alt="PostgreSQL" src="https://img.shields.io/badge/PostgreSQL-Render-4169E1?style=flat-square&logo=postgresql&logoColor=white" />
      </p>
    </td>
    <td width="50%">
      <p><a href="https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-report">upc-pre-202610-1asi0729-12010-VulturesD/nexa-report</a> (This Repository)</p>
      <p>Academic report, product research, backlog, architecture documentation, and project evidence.</p>
      <p><a href="https://github.com/upc-pre-202610-1asi0729-12010-VulturesD/nexa-report">Open Report Repository</a></p>
      <p>
        <img alt="Documentation" src="https://img.shields.io/badge/Documentation-report-0F172A?style=flat-square" />
        <img alt="Open Source" src="https://img.shields.io/badge/Open%20Source-audited%20evidence-0EA5E9?style=flat-square" />
      </p>
    </td>
  </tr>
</table>

---

## Report Structure

| Section | Purpose |
| --- | --- |
| `front-matter/` | Cover, version history, table of contents, collaboration insights, student outcome evidence. |
| `10-chapter-1-introduction/` | Startup profile, solution profile, and target segments. |
| `20-chapter-2-requirements-elicitation/` | Competitors, interviews, needfinding, event storming, ubiquitous language. |
| `30-chapter-3-requirements-specification/` | User stories, impact mapping, and product backlog. |
| `40-chapter-4-product-design/` | Style guidelines, information architecture, UX/UI, DDD architecture, OOD, database design. |
| `50-chapter-5-implementation-validation-deployment/` | SCM, implementation evidence, validation interviews, deployment evidence, product video. |
| `annexes/` | Video, GitHub, deployment, validation, and controlled evidence annexes. |

---

## Team & Domain Ownership

To keep the report aligned with the implementation repositories, evidence areas are assigned to primary owners and support contributors:

| Area | Owner | Support |
| --- | --- | --- |
| **Report Integration & Version History** | DiegoS284 | Cmarin2802, R0obxdnt-bit |
| **Product Research & Requirements** | Cmarin2802 | DiegoS284, GerardRojasMancilla |
| **Architecture & Database Evidence** | GerardRojasMancilla | JoaquinVerde115, DiegoS284 |
| **Implementation & Deployment Evidence** | JoaquinVerde115 | R0obxdnt-bit, DiegoS284 |
| **Annexes, Media & Release Notes** | R0obxdnt-bit | Cmarin2802, DiegoS284 |

---

## Documentation

- [Wiki home](wiki/Home.md)
- [Release notes](docs/releases/README.md)
- [Version history](front-matter/02-version-history.md)
- [Table of contents](front-matter/03-table-of-contents.md)
- [GitHub repository evidence](annexes/annex-d-github-repository-evidence.md)
- [Deployment evidence](annexes/annex-e-deployment-and-services-evidence.md)

---

<p align="center">
  <strong>Nexa Report</strong> · Team Nexa · Universidad Peruana de Ciencias Aplicadas · 2026-10
</p>
